// ================================================================
// app.js — Lógica principal GestorFincas
// ================================================================
...
// ── Formato numérico ─────────────────────────────────────────────
const fmt  = (n, d=0) => !n && n!==0 ? '—' : new Intl.NumberFormat('es-ES',{minimumFractionDigits:d,maximumFractionDigits:d}).format(n);
const fmtE = (n, d=0) => !n ? '—' : fmt(n,d)+' €';
...
function refreshChips(containerId, active) {
  document.querySelectorAll('#'+containerId+' .filter-chip').forEach(c=>c.classList.remove('active'));
...
function getFiltered() {
  ...
  return API.getAll().filter(p => {
    ...
    if (q && ![p.direccion,p.referencia_catastral,p.localidad]
...
function renderList() {
  ...
  if (!filtered.length) {
    container.innerHTML='<div class="empty"><div class="empty-icon">😕</div>No hay inmuebles con esos filtros</div>';
    return;
  }
...
function openDetail(id) {
  ...
  document.getElementById('detail-body').innerHTML = `
    ...
    <div class="card">
      ...
      <div class="row"><span class="lbl">Derecho</span><span class="val">${p.derecho}</span></div>
    ...
    <div class="card">
      ...
      <div class="row"><span class="lbl">Situación</span><span class="val ${p.alquilado?'green':''}">${p.alquilado?'✅ Alquilado':'⬜ Libre'}</span></div>
      ${p.alquilado?`<div class="row"><span class="lbl">Alquiler / mes</span><span class="val green">${fmtE(p.precio_alquiler)}</span></div>`:''}
    </div>
...
function renderForm(p) {
  ...
  document.getElementById('modal-form').innerHTML = `
    <div class="fg">
      ...
      <input class="fi" id="f-dir" value="${p?p.direccion:''}" placeholder="Ej: CL MAYOR 10 Es:1 Pl:02...">
    </div>
    <div class="frow">
      <div class="fg">
        <label class="fl">Localidad *</label>
        <select class="fi fsel" id="f-loc" onchange="chkNuevaLoc(this)">
          ${locOpts}
      ...
      <div class="fg">
        ...
        <select class="fi fsel" id="f-tipo">
    ...
    <div class="fg" id="f-nueva-wrap" style="display:none">
      <label class="fl">Nombre localidad</label>
      <input class="fi" id="f-nueva-loc" placeholder="Nombre de la localidad">
    </div>
    <div class="frow">
    ...
    <div class="fg">
      <label class="fl">Ref. Catastral${St.isNew?' *':''}</label>
      <input class="fi${!St.isNew?' fdim':''}" id="f-ref" value="${p?.referencia_catastral||''}" ${!St.isNew?'readonly':''} placeholder="Referencia catastral">
    </div>`;
}

function chkNuevaLoc(sel) {
  document.getElementById('f-nueva-wrap').style.display = sel.value==='__nueva__'?'block':'none';
}
function toggleAlq() {
...
async function saveProperty() {
  ...
  if (loc==='__nueva__') {
    ...
    if (!loc) { toast('Escribe el nombre de la localidad', true); return; }
  }
  ...
  const obj = {
    ...
    derecho: St.isNew ? '100,00 % de Propiedad' : (API.getAll().find(p=>p.id===ref)?.derecho||'100,00 % de Propiedad'),
    sup_construida: num('f-sup'),
    sup_parcela: St.isNew ? 0 : (API.getAll().find(p=>p.id===ref)?.sup_parcela||0),
    uso: St.isNew ? 'Residencial' : (API.getAll().find(p=>p.id===ref)?.uso||'Residencial'),
    valor_suelo: St.isNew ? 0 : (API.getAll().find(p=>p.id===ref)?.valor_suelo||0),
    valor_construccion: St.isNew ? 0 : (API.getAll().find(p=>p.id===ref)?.valor_construccion||0),
    valor_catastral: num('f-vcat'),
    precio_compra: num('f-compra'),
    precio_venta: num('f-venta'),
    gastos_comunidad: num('f-com'),
    ibi: num('f-ibi'),
    basuras: num('f-bas'),
    alquilado: alq,
    precio_alquiler: alq ? num('f-alq') : 0,
  ...
  try {
    if (St.isNew) {
      if (API.getAll().find(p=>p.id===ref)) { toast('Ya existe esa referencia catastral', true); return; }
...
function renderFinance() {
  const mul = St.period==='anual' ? 12 : 1;
  const sub = St.period==='anual' ? '/año' : '/mes';
  const all = API.getAll();

  let totIng=0, totGst=0;
  all.forEach(p => {
    if (p.alquilado) totIng += (p.precio_alquiler||0);
    totGst += (p.gastos_comunidad||0) + (p.ibi||0)/12 + (p.basuras||0)/12;
  });
  totIng*=mul; totGst*=mul;
  const neto = totIng - totGst;
  const totVC = all.reduce((s,p)=>s+(p.valor_catastral||0),0);
...
// ── Init ─────────────────────────────────────────────────────────
async function init() {
  // Inicializa API con la URL configurada en index.html
